# Blender-Uma-Addon
本插件基于Blender 4.5.7 LTS开发，可用于简化处理从umaviewer导出的赛马娘模型的操作。其包含的功能有骨架分层、生成控制器、快速修复迷你赛马娘模型等。在以下版本中测试插件，功能正常。在4.0.0版本以下，插件一定无法正常运行所有功能。  
...
**烘焙 FK 到 IK**  
仅适用于由以上 *生成 IK* 操作生成的IK控制器。  
如果模型原本就有FK动作，生成IK控制器后FK动作往往会失效，点击烘焙 FK 到 IK 插件就会在场景帧范围内把选中模型的FK动作烘焙到IK控制器上。
...
此节点会在处生成缓存。若原GIF的地址没有变化，则缓存可以复用。
